...
struct PopoverTheme {
    ...
    static let allThemes: [PopoverTheme] = [.playful, .teenageEngineering, .wii, .iPod, makeCustomTheme()]

    private static let themeKey = "selectedThemeName"

    // MARK: - Custom Theme

    private static let customBgKey = "customThemeBg"
    private static let customAccentKey = "customThemeAccent"

    static var customBackground: NSColor {
        get { loadColor(key: customBgKey) ?? NSColor(red: 0.97, green: 0.95, blue: 1.0, alpha: 0.97) }
        set { saveColor(newValue, key: customBgKey) }
    }

    static var customAccent: NSColor {
        get { loadColor(key: customAccentKey) ?? NSColor(red: 0.4, green: 0.3, blue: 0.9, alpha: 1.0) }
        set { saveColor(newValue, key: customAccentKey) }
    }
    ...
    static func makeCustomTheme() -> PopoverTheme {
        let bg = customBackground
        let accent = customAccent
        let isDark = bg.redComponent * 0.299 + bg.greenComponent * 0.587 + bg.blueComponent * 0.114 < 0.5
        let textColor: NSColor = isDark ? .white : NSColor(red: 0.15, green: 0.12, blue: 0.2, alpha: 1.0)
        let dimColor = isDark ? NSColor(white: 0.6, alpha: 1.0) : NSColor(red: 0.5, green: 0.47, blue: 0.53, alpha: 1.0)
        let titleBg = bg.blended(withFraction: 0.08, of: .black) ?? bg
        return PopoverTheme(
            name: "Custom",
            popoverBg: bg, popoverBorder: accent.withAlphaComponent(0.7),
            popoverBorderWidth: 2, popoverCornerRadius: 18,
            titleBarBg: titleBg, titleText: accent,
            titleFont: .systemFont(ofSize: 12, weight: .bold),
            titleFormat: .lowercaseTilde,
            separatorColor: accent.withAlphaComponent(0.2),
            font: .systemFont(ofSize: 12), fontBold: .systemFont(ofSize: 12, weight: .semibold),
            textPrimary: textColor, textDim: dimColor, accentColor: accent,
            errorColor: NSColor(red: 0.9, green: 0.3, blue: 0.2, alpha: 1.0),
            successColor: NSColor(red: 0.3, green: 0.72, blue: 0.5, alpha: 1.0),
            inputBg: bg.blended(withFraction: 0.05, of: isDark ? .white : .black) ?? bg,
            inputCornerRadius: 10,
            bubbleBg: bg.withAlphaComponent(0.95), bubbleBorder: accent.withAlphaComponent(0.5),
            bubbleText: dimColor,
            bubbleCompletionBorder: NSColor(red: 0.3, green: 0.75, blue: 0.5, alpha: 0.7),
            bubbleCompletionText: NSColor(red: 0.2, green: 0.6, blue: 0.4, alpha: 1.0),
            bubbleFont: .systemFont(ofSize: 11, weight: .semibold), bubbleCornerRadius: 12
        )
    }
    ...
    static var current: PopoverTheme {
        get {
            if let saved = UserDefaults.standard.string(forKey: themeKey) {
                if saved == "Custom" { return makeCustomTheme() }
                if let match = allThemes.first(where: { $0.name == saved }) { return match }
            }
            return .playful
        }
        set { UserDefaults.standard.set(newValue.name, forKey: themeKey) }
    }

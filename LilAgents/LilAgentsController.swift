...
class LilAgentsController {
    ...
    /// Detect whether the dock is visible on this screen in any position (bottom, left, or right).
    private func screenHasDock(_ screen: NSScreen) -> Bool {
        let vis = screen.visibleFrame
        let fr = screen.frame
        return vis.origin.y > fr.origin.y   // bottom dock
            || vis.origin.x > fr.origin.x   // left dock
            || vis.maxX < fr.maxX           // right dock
    }
    ...
    func tick() {
        ...
        if vis.origin.x > fr.origin.x {
            // Left dock: characters walk horizontally along the bottom of the content area
            let contentStart = vis.origin.x
            let contentWidth = fr.width - (vis.origin.x - fr.origin.x)
            let (relX, relW) = getDockIconArea(screenWidth: contentWidth)
            dockX = contentStart + relX
            dockWidth = relW
            dockTopY = fr.origin.y + 15   // align characters to screen bottom edge
        } else if vis.maxX < fr.maxX {
            // Right dock: characters walk horizontally along the bottom of the content area
            let contentWidth = vis.maxX - fr.origin.x
            let (relX, relW) = getDockIconArea(screenWidth: contentWidth)
            dockX = fr.origin.x + relX
            dockWidth = relW
            dockTopY = fr.origin.y + 15   // align characters to screen bottom edge
        } else {
